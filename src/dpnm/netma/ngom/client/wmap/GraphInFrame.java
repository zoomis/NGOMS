...
class GraphInFrame extends JInternalFrame {
	...
	static Dimension GRAPH_SIZE = new Dimension(600, 500);
	...
	GraphInFrame(DeviceInfo deviceInfo, InterfaceInfo interfaceInfo, int type) {
		super(interfaceInfo.getIfDescr() + "@" + deviceInfo.getHost() + " [graph] ("+
                interfaceInfo.getIfAlias()+")",
                true,true,true,true);
        
        //  decide location of internal frame
        setLocation(WeatherMapClient.getNumWindow() * 30, WeatherMapClient.getNumWindow() * 30) ;
...
//		setVisible(true);
	...
	GraphInFrame(DeviceInfo deviceInfo, int type) {
		super(deviceInfo.getHost() + " [graph]", true,true,true,true);
...
//		setVisible(true);
	...
	private void createGraph() {
		...
		try {
			graphBytes = DataManager.getInstance().getPngGraph(deviceInfo, interfaceInfo, start, end);
